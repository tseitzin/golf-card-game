...
import { RaceConfig, PlayerConfig } from '../../../types/race';
import { CAR_COLORS, GAME_CONFIG } from '../../../constants/race/index.ts';
...
export function SetupScreen({ config, onConfigChange, onStartRace }: SetupScreenProps) {
	...
	const handleHumanPlayersChange = (count: number) => {
		...
		onConfigChange({ ...config, humanPlayers: count, playerConfigs: newConfigs });
	};
		...
		return (
			<div 
				...
				style={{
					background: 'linear-gradient(to bottom right, #2C3E50, #34495E, #1A252F)'
				}}
			>
